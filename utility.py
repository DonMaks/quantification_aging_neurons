...
def readSWC(filename, as_list=False):
    '''Read an .swc file into a list of lists.'''
...
def findRoots(tree, return_node=False):
    ...
    root_nodes = []
    for index in roots:
        node = tree[np.where(tree[:,6]==index)].reshape(7).tolist()
...
def findSomaNodes(tree, scale=(1,1,1), return_list=False):
    soma_diameter_threshold = 24*scale[0]
    soma_nodes = []
...
def dist3DWithRadius(start, end, scale=(1,1,1)):
    ...
    dist = np.sqrt(((start[2]-end[2])*scale[0])**2+((start[3]-end[3])*scale[1])**2+((start[4]-end[4])*scale[2])**2)-(start[5]+end[5])
    return dist
